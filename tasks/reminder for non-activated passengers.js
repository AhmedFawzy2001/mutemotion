...
const passengerUser = require("../models/passengerModel"); // Import the User model
...
const sendActivationRemindersForPassengers = () => {
  cron.schedule('*/3 * * * *', async () => {
    try {
      const nonActivatedUsers = await passengerUser.find({ isActive: false, createdAt: { $lt: new Date(Date.now() - 3 * 60 * 1000) } });
...
// Export the function
module.exports =  sendActivationRemindersForPassengers;
